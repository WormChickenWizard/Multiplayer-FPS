...
public class Weapon : MonoBehaviour
{
    ...
    void Start()
    {
        //disables the crosshairs and ammo counter for the weapons that don't belong to the local player
        if (!GetComponentInParent<NetworkIdentity>().isLocalPlayer)
        {
            showCrosshair = false;
            showCurrentAmmo = false;
        }
    ...
    void Fire()
    {
        ...
        for (int i = 0; i < shotPerRound; i++)
        {
            ...
            {
                Debug.DrawRay(raycastStartSpot.position, direction, Color.green, 1f);

                Debug.Log("Local Player Hit: " + hit.collider.gameObject);
                ...
                try
                {
                GetComponentInParent<WeaponSystem>().CmdChangeHealth(-damage, hit.collider.gameObject.GetComponentInParent<Health>().gameObject); 
                }
#pragma warning disable 0168
                catch (System.NullReferenceException e)
                {
                    //if an non health-monitored object is hit, continue
                }
#pragma warning restore 0168
                ...
                if (shooterAIEnabled)
                {
                    hit.collider.transform.SendMessageUpwards("Damage", damage / 100, SendMessageOptions.DontRequireReceiver);
                }
            ...
            {
                Debug.DrawRay(raycastStartSpot.position, direction, Color.red, 1f);
            }
        }
